...
export type SeedSummary = {
  ...
  traps: number;
  visits: number;
  poisonAdditions: number;
};

export async function runDemoSeed(
  log: SeedLogger = (msg) => console.log(msg)
): Promise<SeedSummary> {
  ...
  try {
    ...
    for (const cust of DEMO_CUSTOMERS) {
      log(`Creating customer: ${cust.businessName}`);
      ...
      for (const siteSpec of cust.sites) {
        ...
        for (const woSpec of siteSpec.workOrders) {
          ...
          for (let i = 0; i < woSpec.trapCount; i++) {
            ...
            const [trap] = await db
              ...
              .values({
                ...
                status:
                  i === woSpec.trapCount - 1 && woSpec.trapCount > 3
                    ? "damaged"
                    : "active",
                installedBy: TECH_ID,
              })
          ...
          for (let v = 0; v < visitDays.length; v++) {
            ...
            let runningRemaining = 100;

            for (const t of createdTraps) {
              if (t.label === "T01") continue;

              const qty =
                baseConsumption + ((parseInt(t.id.slice(0, 2), 16) % 5) - 2);
    ...
    return {
      ...
      traps: totalTraps,
      visits: totalVisits,
      poisonAdditions: totalPoisonAdditions,
    };
  } finally {
    await client.end();
  }
